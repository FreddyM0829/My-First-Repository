...
#Graph of cities with connections to each city. similar to our class exercise 
...
class SearchAlgorithms:
    def breadthFirstSearch(self, start, goal, graph):
        ...
        will propose to go to to reach the goal, and (ii) set of expanded cities (visited nodes). 
    ...
    def depthFirstSearch(self, start, goal, graph):
        ...
        "*** YOUR CODE HERE ***"
        visited = set()
        fringe = [(start , [start])] #----> [(node , [complete path]}]             #!!!For better learning of Python try and use DEQUE!!!
        
        while fringe :
            (node, path) = fringe.pop()
            #will only use if found the goal state
            if node == goal:
                return ("Returned solution: ", path, ", Expanded cities: ", visited)
            
            #will add the new cities to the node by appending"adding new spot"
            if node not in visited:
                visited.add(node)
                #will be reversed because it will be LIFO (Last In First Out)
    ...
    def uniformCostSearch(self, start, goal, graph, weights):
        ...
        only has one directional weights. In the class, we mentioned that there is a path from Arad to Sibiu and back. If the exception
        occurs, you will need to get the weight of the nodes in reverse direction (weights[(neighbor, node)]) """
        "*** YOUR CODE HERE ***"
        ...
        while not queue.empty():
            ...
            if node == goal:
                return("Returned Solutions: ", path, "Expanded cities: ", visited)
            
            if node not in visited:
                visited.add(node)
                successors = (graph[node])
                for neighbor in successors:
                    if (node, neighbor) not in weights:
                        new_cost = cost + weights[(neighbor, node)] 
                    else:
    ...
    def AStar(self, start, goal, graph, weights, heuristic):
        ...
        Important things to remember
        1 - Use PriorityQueue with .put() and .get() functions
        2 - In addition to putting the start or current node in the queue, and the
        g(n), also put the combined cost (i.e., g(n) + h(n))
...
print("Breadth First Search Result") # ['San Bernardino', 'Rancho Cucamonga', 'LosAngeles']
pprint(search.breadthFirstSearch('San Bernardino', 'Los Angeles', graph))

print("\nDepth First Search Result") # ['San Bernardino', 'Riverside', 'Ontario','Whittier', 'Pomona', 'Azusa', 'Rancho Cucamonga', 'Los Angeles']
pprint(search.depthFirstSearch('San Bernardino', 'Los Angeles', graph)) #Here we have the start city San Bernardino and end city Los Angeles

print("\nUniform Cost Search Result") # ['San Bernardino', 'Rancho Cucamonga', 'LosAngeles']
pprint(search.uniformCostSearch('San Bernardino', 'Los Angeles', graph, weights))

print("\nA* Search Result") # ['San Bernardino', 'Rancho Cucamonga', 'Los Angeles']
